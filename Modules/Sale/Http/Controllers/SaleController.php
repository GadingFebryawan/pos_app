...
class SaleController extends Controller
{
    ...
    public function create()
    {
        ...
        $listChannel = $this->pgService->getListChannelVendor();

        return view('sale::create', compact('listChannel'));
    }
    ...
    public function store(StoreSaleRequest $request)
    {
        try {
            ...
            $saleID = null;
            DB::transaction(function () use ($request, &$saleID, &$userCompanyIds) {
                ...
                $sale = Sale::create([
                    'date' => $request->date,
                    'exp_date' => !empty($request->exp_date) ? Carbon::parse($request->exp_date)->timezone('Asia/Jakarta')->format('Y-m-d H:i:s') : null,
    ...
    public function storeByXlsx(Request $request)
    {
        try {
            $userCompanyIds = Auth::user()->companies()->pluck('companies.id')->toArray();
            if (count($userCompanyIds) > 1) {
            ...
            $request->validate([
                'file_xlsx' => 'required|file|mimes:xlsx,xls',
                'notes' => 'required|string',
            ]);

            $notes = $request->input('notes');
            Excel::import(new SalesImport($notes), $request->file('file_xlsx'));

            toast('Import Sales Successfull', 'success');
            return redirect()->route('sales.index');
        } catch (\Throwable $e) {
            Log::error('Sale import creation failed', ['error' => $e->getMessage() . ",at " . $e->getFile() . ",line " . $e->getLine()]);
            toast($e->getMessage() ?? 'Sale import creation failed. Please try again.', 'error');
            return redirect()->back();
        }
    }
    ...
    {
        $filePath = public_path('template/template_sales.xlsx');

        if (!file_exists($filePath)) {
            abort(404, 'Template file not found.');
        }

        return response()->download($filePath, 'template_import_sales.xlsx', [
